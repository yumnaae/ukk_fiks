        ...
        <div class="overflow-hidden sm:rounded-lg">
            <div class="flex justify-between items-center mb-4">
                ...
                <div class="flex justify-start w-full">
                    <form action="{{ route('pkl.index') }}" method="GET" class="w-1/5 relative">
                ...
                <a href="{{ route('pkl.create') }}" class="h-8 w-12 bg-white border border-gray-700 text-gray-700 rounded-md hover:shadow-sm flex items-center justify-center transition-all duration-200">
                    <svg xmlns="http://www.w3.org/2000/svg" 
            ...
            @if (session()->has('error'))
                <div class="bg-red-600 bg-opacity-80 text-sm text-white border border-red-400 p-2 rounded mb-4">
                    {{ session('error') }}
                </div>
            @endif
